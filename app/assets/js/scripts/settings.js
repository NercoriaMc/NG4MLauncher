        ...
        if (Object.prototype.hasOwnProperty.call(queryMap, 'error')) {
            ...
            AuthManager.addMicrosoftAccount(authCode).then(value => {
                ...
                .catch((displayableError) => {
                    ...
                    switchView(getCurrentView(), viewOnClose, 500, 500, () => {
                        setOverlayContent(actualDisplayableError.title, actualDisplayableError.desc, "Essayer à nouveau")
                        setOverlayHandler(() => {
                            toggleOverlay(false)
                        })
                        toggleOverlay(true)
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            ...
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Choisie: Java ${v.version.major} Update ${v.version.update} (x${v.arch})${vendor}`
            } else {
                settingsJavaExecDetails.innerHTML = `Choisie: Java ${v.version.major}.${v.version.minor}.${v.version.revision} (x${v.arch})${vendor}`
            }
        } else {
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/NercoriaMc/NG4MLauncher/releases.atom',
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Download from GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le Launcher et exécutez le dmg pour mettre à jour.</span>', false, () => {
